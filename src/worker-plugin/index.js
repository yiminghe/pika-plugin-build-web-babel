...
function resolveRequire(name, extensions, paths) {
  let ret;

  for (const e of extensions) {
    try {
      ret = require.resolve(name + e, paths);
    } catch (e) {
...
module.exports = function workerLoaderPlugin(config = null) {
  ...
  let extensions = config && config.hasOwnProperty('extensions') ? config.extensions : [];
      ...
      if (!projectOptions) {
        ...
        if (options.plugins && options.plugins.length) {
          ...
          if (typeof options.input === 'string') {
            try {
              const entry = resolveRequire(options.input, extensions, { paths: [cwd] });
            ...
            for (let i = 0, n = options.input.length; i < n; ++i) {
              try {
                const entry = resolveRequire(options.input[i], extensions, { paths: [cwd] });
            ...
            for (let i = 0, n = keys.length; i < n; ++i) {
              ...
              try {
                const entry = resolveRequire(input, extensions, { paths: [cwd] });
                const entryPath = path.dirname(entry);
                const components = entryPath.split(path.sep);
                if (components.length < componentCount) {
                  componentCount = components.length;
      ...
      if (importee === 'rollup-plugin-web-worker-loader::helper') {
        ...
        if (!idMap.has(name)) {
          ...
          if (importer) {
            ...
            paths.push(folder);
            target = resolveRequire(name, extensions, { paths });
